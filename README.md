...
- 依存関係がインストールされていること（`npm install` を実行済み）
- `.dev.vars` ファイルにSpotify API認証情報が設定されていること（詳細は[環境変数の設定](#環境変数の設定)を参照）
...
バックエンドサーバーが `http://localhost:8787` で起動します。
...
1. バックエンドのヘルスチェック: `http://localhost:8787/` にアクセス
